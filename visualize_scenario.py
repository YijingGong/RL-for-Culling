...
FIGURES_ROOT = Path("prod_outputs") / "original_figures"
TABLES_ROOT = Path("prod_outputs") / "original_tables"

SCENARIOS = ['2025', 'OG', 'OB', 'UG', 'UB']
SEEDS = [42, 123, 456, 789, 1024]
MODEL_PREFIX = "DQN_prod"      # production (5-state) model file prefix; use "DQN" for the base model
REFERENCE_PROD_LEVEL = 1.0     # production level at which policy/value figures are drawn (average cow)
...
def collapse_prod_level(q_table, ref=REFERENCE_PROD_LEVEL):
    """Collapse a production-model Q-table (5-tuple keys incl. production level)
    down to 4-tuple keys at the reference production level, so the rest of this
    script's 4-tuple lookups work unchanged. Base-model 4-tuple Q-tables pass through.
    """
    if not q_table:
        return q_table
    sample = next(iter(q_table))
    if len(sample) == 4:
        return q_table  # base model, already 4-tuple keyed
    levels = sorted({k[4] for k in q_table})
    pick = ref if any(abs(l - ref) < 1e-9 for l in levels) else min(levels, key=lambda x: abs(x - ref))
    return {k[:4]: v for k, v in q_table.items() if abs(k[4] - pick) < 1e-9}


def load_pkl(filename):
    with open(filename, "rb") as f:
        q_table, rewards, epsilon = pickle.load(f)
    q_table = collapse_prod_level(q_table)
...
def auto_discover_files(collected_dir, scenario=None):
    ...
    for sc in scenarios:
        ...
        for seed in SEEDS:
            for pattern in [
                os.path.join(collected_dir, f"{MODEL_PREFIX}_{sc}_seed{seed}.pkl"),
                os.path.join(collected_dir, f"{MODEL_PREFIX}_{sc}_{seed}.pkl"),
            ]:
...
def main():
    ...
    figures_dir = Path(args.figures_dir) if args.figures_dir else FIGURES_ROOT

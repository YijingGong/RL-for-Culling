...
OUTPUT_DIR="outputs"                     # (legacy) kept for any intermediate artifacts
PROD_DIR="prod_outputs"                  # All revision deliverables:
                                         #   prod_outputs/                 -> new Figs 7 & 8
                                         #   prod_outputs/original_figures -> regenerated manuscript figures
                                         #   prod_outputs/original_tables  -> regenerated manuscript tables
...
if [ -d "${COLLECTED_DIR}" ]; then
    existing_pkl=$(find "${COLLECTED_DIR}" -maxdepth 1 -name "${MODEL_PREFIX}_*.pkl" ! -name "*_eval.pkl" 2>/dev/null | wc -l | tr -d ' ')
fi
...
if [ "$existing_pkl" -gt 0 ]; then
    ...
    tarball_count=0
    for tarball in "${TARBALL_DIR}"/${MODEL_PREFIX}_*_results.tar.gz; do
        if [ -f "$tarball" ]; then
    ...
    for tarball in "${TARBALL_DIR}"/${MODEL_PREFIX}_*.tar.gz; do
...
for scenario in "${SCENARIOS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        if [ -f "${COLLECTED_DIR}/${MODEL_PREFIX}_${scenario}_seed${seed}.pkl" ] || \
           [ -f "${COLLECTED_DIR}/${MODEL_PREFIX}_${scenario}_${seed}.pkl" ]; then
            found=$((found + 1))
        else
            echo -e "${RED}  MISSING: ${MODEL_PREFIX}_${scenario}_seed${seed}.pkl${NC}"
            missing=$((missing + 1))
        fi
    done
done
...
for scenario in "${SCENARIOS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        ...
        eval_csv="${COLLECTED_DIR}/${MODEL_PREFIX}_${scenario}_seed${seed}_eval.csv"
        eval_pkl="${COLLECTED_DIR}/${MODEL_PREFIX}_${scenario}_seed${seed}_eval.pkl"
...
mkdir -p "${PROD_DIR}/original_tables"

python3 aggregate_results.py \
    --results_dir "${COLLECTED_DIR}/" \
    --output "${PROD_DIR}/original_tables/aggregated_results.csv" \
    --tables_dir "${PROD_DIR}/original_tables"
...
###############################################################################
# STEP 7: Production-level figures (always runs)
#   Fig 8 = new main figure (plain + contour); Fig 7 = appendix.
#   Written to prod_outputs/ (root), separate from the original_figures/.
###############################################################################
echo -e "${YELLOW}STEP 7: Production-level figures (Figs 7 & 8)${NC}"

python3 visualize_production.py \
    --collected_dir "${COLLECTED_DIR}/" \
    --outdir "${PROD_DIR}" \
    --cmap YlGnBu
...
echo ""
echo "  ${PROD_DIR}/"
echo "  ├── 7_hurdle_map_<SC>.png            (appendix: alternative projection)"
echo "  ├── 8_stay_map_<SC>.png              (NEW main figure, plain)"
echo "  ├── 8_stay_map_<SC>_contour.png      (NEW main figure, with contours)"
echo "  ├── 1..6_*.png                       (supporting production figures)"
echo "  ├── original_figures/"
echo "  │   ├── cross_scenario_summary.png   (manuscript Fig 1 / Fig 5)"
echo "  │   └── <SC>_*.png                    (manuscript Figs 2, 3, 4)"
echo "  └── original_tables/"

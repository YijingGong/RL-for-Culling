...
def parse_cull_meat_prices(block_text):
    ...
    """
    rows = []
    # Use a block-level regex to capture patterns like:
    # "Breaker - 75% Lean 500 lbs and up 231.40-235.44 233.63"
    pat = re.compile(
        r'\b(Breaker|Boner|Cutter)\b[^\n]{0,120}?(?:([0-9]{1,3}%\s*Lean)[^\n]{0,20})?'
        r'[^\n]{0,40}?500\s*lbs(?:\s*(?:and)?\s*up)?[^\n]{0,40}?'
        r'([0-9\.,]+\s*(?:to|\-|–|—)\s*[0-9\.,]+)(?:\s+([0-9\.,]+))?',
        re.I,
    )

    for m in pat.finditer(block_text):
        category = m.group(1).title()
        lean = m.group(2).strip() if m.group(2) else None
        range_text = m.group(3)
        avg_text = m.group(4)

        low = high = avg = None
        if range_text:
            # normalize and split on hyphen or 'to'
            rr = re.split(r'\s*(?:to|\-|–|—)\s*', range_text)
            if len(rr) >= 2:
                low = clean_num(rr[0])
                high = clean_num(rr[1])
        if avg_text:
            avg = clean_num(avg_text)
        # if no avg but low/high present and we want avg, compute midpoint

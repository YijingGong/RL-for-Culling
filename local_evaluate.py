...
def evaluate_policy(q_table, env, num_episodes=1000, max_steps=180, gamma=0.95,
    ...
    total_involuntary = 0
    total_steps = 0
    prod_sum = 0.0          # accumulates production level M of the cow occupying the stall
    replacements_per_ep = []
    ...
    for ep in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            total_steps += 1
            prod_sum += state[4]        # production level of the cow currently in the stall
    ...
    mean_production = prod_sum / total_steps if total_steps > 0 else 0   # time-averaged M of cows in the herd
...
def save_evaluation_csv(results, parity_results, csv_path, steady_state=None):
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        if steady_state is not None:
            ...
            writer.writerow(['mean_production', steady_state.get('mean_production', '')])
    ...
    if args.single:
        ...
        for scenario in SCENARIOS:
            for seed in SEEDS:
                ...
                patterns = [
                    f"{MODEL_PREFIX}_{scenario}_seed{seed}.pkl",
                    f"{MODEL_PREFIX}_{scenario}_{seed}.pkl",
                ]
                ...
                if pkl_path is None:
                    ...
                    matches = glob.glob(os.path.join(args.collected_dir,
                                                     f"{MODEL_PREFIX}_{scenario}*seed{seed}*.pkl"))
                ...
                if args.skip_existing:
                    eval_csv = os.path.join(args.collected_dir,
                                            f"{MODEL_PREFIX}_{scenario}_seed{seed}_eval.csv")
                    eval_pkl = os.path.join(args.collected_dir,
                                            f"{MODEL_PREFIX}_{scenario}_seed{seed}_eval.pkl")

...
for i in $(seq 1 $NUM_RUNS); do
    ...
    echo "Timestamp: $(date)"
    echo "Previous scenario: $PREVIOUS_SCENARIO"
    echo "Use file: ${OUTPUT_DIR}/DQN_${PREVIOUS_SCENARIO}_500k_run${i}_continue_${SCENARIO}.pkl"
    echo "=========================================="
    
    # Run the training with the scenario parameter
    python dqn_learning.py \
        --filename "${OUTPUT_DIR}/DQN_${PREVIOUS_SCENARIO}_500k_run${i}_continue_${SCENARIO}.pkl" \
        --episodes $EPISODES \
        --scenario $SCENARIO

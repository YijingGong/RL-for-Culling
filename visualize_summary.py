...
REFERENCE_PROD_LEVEL = 1.0     # production level at which Q-table figures are drawn (average cow)
...
def collapse_prod_level(q_table, ref=REFERENCE_PROD_LEVEL):
    """Collapse a production-model Q-table (5-tuple keys incl. production level) to
    4-tuple keys at the reference production level; base-model 4-tuple tables pass through."""
    if not q_table:
        return q_table
...
def auto_discover_files(collected_dir):
    ...
    for scenario in SCENARIOS:
        ...
        for seed in SEEDS:
            for pattern in [
                os.path.join(collected_dir, f"{MODEL_PREFIX}_{scenario}_seed{seed}.pkl"),
                os.path.join(collected_dir, f"{MODEL_PREFIX}_{scenario}_{seed}.pkl"),
            ]:
...
def auto_discover_eval_files(collected_dir):
    scenario_eval_files = {}
    for scenario in SCENARIOS:
        pattern = os.path.join(collected_dir, f"{MODEL_PREFIX}_{scenario}_seed*_eval.pkl")
...
def main():
    ...
    output_dir = Path(args.output_dir) if args.output_dir else FIGURES_ROOT
